	...
	<body>
		...
		<script>
			$.loadGoodsMessageList = function(orderStatus) {
				$.ajax({
					type: "post",
					url: "<?php echo U('admin/loadGoodsMessageList');?>",
					async: true,
					data: {
					...
					success: function(d) {
						...
						if(data.code == "0000") {
							...
							data.data.forEach(function(arg, index) {
								...
								tempStr.push('<img src=' + arg.coverimg + ' width="80">');
								tempStr.push('</div>');
								...
								tempStr.push('<div class="item-text">' + arg.veildescription + '</div>');							
								tempStr.push('<div class="item-title-row">');
								tempStr.push('<div class="item-title">男套：' + arg.manveil + ' 女套：' + arg.feveil + '</div>');
								tempStr.push('</div>');
								tempStr.push('</div>');
								tempStr.push('</label>');
								tempStr.push('</div>');
								tempStr.push('<div class="swipeout-actions-right">');
								tempStr.push('<a onclick="$.deleteGood(\'' + arg.id + '\');" class="bg-danger swipeout" href="#">删除套餐</a>');
								tempStr.push('</div>');
								tempStr.push('</li>');
							});
							$('#userContainer').empty();
			...
			$.editGood = function(goodID) {
					window.location.href = "http://localhost/hsylglxt/admin.php/index/editGood.html?goodID=" + goodID;
				}
