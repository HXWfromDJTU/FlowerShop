	...
	<body>
		...
		<script>
						...
						success: function(d) {
							...
							if(data.code == "0000") {
								...
								data.data.forEach(function(arg, index) {
									tempStr.push('<li  id="user' + arg.id + '"   class="swipeout outter">');
									tempStr.push('<div onclick="$.editUser(\'' + arg.id + '\');" class="swipeout-content">');
									...
									tempStr.push('<div class="item-title-row">');
									tempStr.push('<div class="item-title">生日：'+arg.birthday+'</div>');
									tempStr.push('</div>');
									tempStr.push('</div>');
									tempStr.push('</label>');
									tempStr.push('</div>');
			...
			$.editUser = function(userID){
				window.location.href="http://localhost/ffhysc/admin.php/index/editUser.html?userID="+userID;
			}
